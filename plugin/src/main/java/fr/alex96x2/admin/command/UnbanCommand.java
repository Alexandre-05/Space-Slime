...
public class UnbanCommand implements CommandExecutor {
    ...
    public UnbanCommand(AdminPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (plugin.getBanService().findActive(target.getUniqueId()).isEmpty()) {
            plugin.getMessageService().send(sender, "not-banned", Map.of("player", targetName));
            return true;
        }

        plugin.getBanService().unban(target.getUniqueId(), sender.getName(), () ->
                plugin.getMessageService().send(sender, "unban-success", Map.of("player", targetName)));
        return true;
    }
}
