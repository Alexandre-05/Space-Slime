...
public class MuteListCommand implements CommandExecutor {
    ...
    public MuteListCommand(AdminPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        int total = plugin.getMuteService().countActive();
        int totalPages = Math.max(1, (int) Math.ceil(total / (double) PAGE_SIZE));
        List<SanctionRecord> records = plugin.getMuteService().listActive(page, PAGE_SIZE);

        plugin.getMessageService().send(sender, "mutelist-header", Map.of(
        ...
        for (SanctionRecord record : records) {
            ...
            plugin.getMessageService().send(sender, "mutelist-entry", Map.of(
                    "player", name, "reason", record.reason(),
